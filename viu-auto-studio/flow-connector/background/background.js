...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === 'DOWNLOAD_FILE') {
        (async () => {
            try {
                // When Viu owns the Factory run, its adapter streams the blob
                // directly to the project-bound FastAPI ingest endpoint. Do
                // not also create a user-visible Chrome download/history item.
                const viuRuntime = await chrome.storage.local.get(['paired', 'autoFactory', 'apiBaseUrl']);
                if (viuRuntime.paired && viuRuntime.autoFactory && viuRuntime.apiBaseUrl) return;
