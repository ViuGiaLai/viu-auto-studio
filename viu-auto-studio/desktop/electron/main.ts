      ...
      try {
        ...
        if (process.env.VIU_UI_SMOKE === "1") {
          ...
          const readUiState = async () => mainWindow.webContents.executeJavaScript(`(() => ({
            url: location.href,
            tabs: [...document.querySelectorAll('button')].map((node) => ({ text: node.textContent || "", selected: node.getAttribute("aria-selected") })),
        ...
        if (process.env.VIU_SETTINGS_WORKFLOW_SMOKE === "1") {
          ...
          await waitSettings(1200)
          const initialSettingsState = await mainWindow.webContents.executeJavaScript(`(() => { const labels = ['Thiết lập nhanh','Nội dung & AI','Giọng & Âm thanh','Hình ảnh & Video','Dựng & Xuất video','Tài khoản & Kết nối','Hiệu năng','Nâng cao']; const buttons = [...document.querySelectorAll('button')]; return { labelsFound: labels.filter((label) => buttons.some((node) => (node.textContent || '').includes(label))), body: document.body.innerText || '' }; })()`)
          if (initialSettingsState.labelsFound.length < 8) throw new Error('Settings workflow không render đủ tab; tabs=' + initialSettingsState.labelsFound.join('|'))
          ...
          for (const label of tabLabels) {
            ...
            const state = await mainWindow.webContents.executeJavaScript(`(() => { const matches = [...document.querySelectorAll('button')].filter((node) => (node.textContent || '').includes(${JSON.stringify(label)})); const target = matches[0]; return { selected: matches.find((node) => node.getAttribute('data-state') === 'active' || node.getAttribute('aria-selected') === 'true')?.getAttribute('data-state') || matches.find((node) => node.getAttribute('data-state') === 'active' || node.getAttribute('aria-selected') === 'true')?.getAttribute('aria-selected') || '', bodyHasLabel: (document.body.innerText || '').includes(${JSON.stringify(label)}), matches: matches.map((node) => ({ text: node.textContent || '', role: node.getAttribute('role'), state: node.getAttribute('data-state'), selected: node.getAttribute('aria-selected'), rect: Boolean(node.getClientRects().length) })) }; })()`)
          ...
          await mainWindow.webContents.executeJavaScript(`(() => { const target = [...document.querySelectorAll('button')].find((node) => (node.textContent || '').includes('Dựng & Xuất video')); if (!target) throw new Error('Không tìm thấy tab Dựng & Xuất video'); target.focus(); target.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, button: 0, buttons: 1 })); target.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, cancelable: true, button: 0 })); target.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, button: 0 })); return true })()`)
          console.log('[SettingsSmoke] edit-clicked')
          await waitSettings(500)
          console.log('[SettingsSmoke] edit-ready')
          const originalPreset = String(await mainWindow.webContents.executeJavaScript(`(() => { const labels = ['YouTube ngang','Shorts / TikTok','Video vuông','Chất lượng cao']; const text = Array.from(document.querySelectorAll('button[aria-pressed="true"]')).map((node) => String(node.textContent || '')).find((value) => labels.some((label) => value.includes(label))); return String(text || 'YouTube ngang'); })()`))
          console.log('[SettingsSmoke] preset-read')
          const originalSubtitle = String(await mainWindow.webContents.executeJavaScript(`(() => { const labels = ['Nổi bật','Cơ bản','Karaoke']; const text = Array.from(document.querySelectorAll('button[aria-pressed="true"]')).map((node) => String(node.textContent || '')).find((value) => labels.some((label) => value.includes(label))); return String(text || 'Nổi bật'); })()`))
